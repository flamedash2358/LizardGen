...
if config["fun"]["april_fools"]:
    config["fun"]["newborns_can_roam"] = True
...
def save_clanlist(loaded_clan=None):
    ...
    for clan_name in switches['clan_list']:
...
def save_cats():
    ...
    save_faded_cats(clanname)  # Fades cat and saves them, if needed
    ...
    for cat in cat_to_fade:
        ...
        for x in inter_cat.get_parents():
            if x in cat_class.all_cats:
                ...
                parent_faded = add_faded_offspring_to_faded_cat(x, cat)
                if not parent_faded:
...
def add_faded_offspring_to_faded_cat(parent, offspring):
    ...
    safe_save(
        f"{get_save_dir()}/{clan.name}/faded_cats/{parent}.json", cat_info
    )
